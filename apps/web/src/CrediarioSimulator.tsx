...
const creditPhases:Record<CreditPlan,readonly string[]>={
 CT1:["Iniciando sessão da Plataforma Click","Localizando o produto","Adicionando o produto principal","Adicionando 447164","Adicionando 801911","Verificando / aplicando garantia","Selecionando o seguro prestamista","Enviando CPF","Configurando CT1 sem entrada","Calculando parcelas","Finalizando consulta"],
 CT2:["Iniciando sessão da Plataforma Click","Localizando o produto","Adicionando o produto principal","Adicionando 447164","Adicionando 801911","Verificando / aplicando garantia","Selecionando o seguro prestamista","Enviando CPF","Configurando CT2","Aplicando entrada variável","Calculando parcelas","Finalizando consulta"],
 "48":["Iniciando sessão da Plataforma Click","Localizando o produto","Adicionando o produto principal","Adicionando 447157","Adicionando 801911","Verificando / aplicando garantia","Selecionando o seguro prestamista","Enviando CPF","Configurando Cliente Novo 48","Aplicando entrada variável","Calculando parcelas","Finalizando consulta"]
};
...
export function CrediarioSimulator(){
 ...
 const [loading,setLoading]=useState(false),[searching,setSearching]=useState(false),[progressIndex,setProgressIndex]=useState(-1);
 ...
 const phases=creditPhases[plan];
 useEffect(()=>{if(!loading){setProgressIndex(-1);return}setProgressIndex(0);const timer=window.setInterval(()=>setProgressIndex(current=>Math.min(current+1,phases.length-2)),1100);return()=>window.clearInterval(timer);},[loading,plan,phases.length]);
 async function searchByCode(){const clean=code.trim();setError("");setResult(null);if(!clean){setError("Digite o código do produto.");return null}setSearching(true);try{const p=await lookupClickProduct(clean);if(!p.found){setError("Produto não encontrado na Plataforma Click.");return null}setLoadedProduct(p);addProduct(p as any);const opts=p.voltageOptions||[];if(!voltage&&opts.length===1){const v=opts[0].match(/110|127|220/)?.[0];if(v)setVoltage(v==="127"?"110":v)}return p}catch(e){setError(friendlyError(e));return null}finally{setSearching(false)}}
 async function simulate(){const resultTab=window.open("about:blank","_blank");setError("");setResult(null);const cleanCode=code.trim();if(!cleanCode){setError("Digite o código do produto.");return}const cleanCpf=cpf.replace(/\D/g,"");if(cleanCpf.length!==11){setError("Digite um CPF com 11 números.");return}if(!Number.isInteger(auxiliaryQuantity)||auxiliaryQuantity<1||auxiliaryQuantity>20){setError("A quantidade do produto auxiliar deve ficar entre 1 e 20.");return}if((plan==="CT2"||plan==="48")&&!(entryNumber(entry)>0)){setError(`${plan} exige uma entrada maior que zero.`);return}setLoading(true);try{let p=product;if(!p){p=await lookupClickProduct(cleanCode);if(!p.found)throw new Error("PRODUCT_NOT_FOUND");setLoadedProduct(p);addProduct(p as any)}const data=await simulateClickCredit({productCode:cleanCode,plan,installments,...(plan==="CT2"||plan==="48"?{entry:entryNumber(entry)}:{}),...(voltage?{voltage}:{}),warranty,cpf:cleanCpf,auxiliaryQuantity});setProgressIndex(phases.length-1);setResult(data);if(data?.clickUrl&&resultTab){resultTab.location.replace(data.clickUrl)}else if(resultTab){resultTab.close()}}catch(e){if(resultTab)resultTab.close();setError(friendlyError(e))}finally{setLoading(false)}}
 ...
 return <section className="creditSimulator">
  ...
  <div className="creditGrid">
   <label className="cpfField">Código do produto<div className="creditCodeRow"><input value={code} onChange={e=>{setCode(e.target.value);setLoadedProduct(null)}} inputMode="numeric" placeholder="Ex.: 412100"/><button type="button" onClick={searchByCode} disabled={searching||loading}>{searching?"Buscando...":"Buscar código"}</button></div></label>
  ...
  {loading&&<div className="creditProgress" aria-live="polite"><div className="creditProgressTop"><strong>{currentPhase}</strong><span>{progressPercent}%</span></div><div className="creditProgressBar"><span style={{width:`${progressPercent}%`}}/></div><div className="creditProgressList">{phases.map((phase,index)=><div key={phase} className={index<progressIndex?"done":index===progressIndex?"active":"pending"}><span>{index<progressIndex?"✓":index===progressIndex?"●":"○"}</span>{phase}{phase.includes("801911")&&<b>{auxiliaryQuantity}x</b>}</div>)}</div></div>}
  ...
  {result&&<div className="creditResult creditReceipt"><div><small>Plano</small><strong>{result.plan}</strong></div><div><small>Entrada</small><strong>{result.entry!==undefined?`R$ ${money(result.entry)}`:"—"}</strong></div><div><small>{result.plan==="CT1"?"Parcelas":"Composição"}</small><strong>{result.plan!=="CT1"?`1 entrada + ${result.installments}x de R$ ${money(result.installmentValue)}`:result.installmentValue!==undefined?`${result.installments}x de R$ ${money(result.installmentValue)}`:`${result.installments}x — valor não capturado`}</strong></div>{result.financedTotal!==undefined&&<div><small>Total parcelado</small><strong>R$ {money(result.financedTotal)}</strong></div>}<div><small>Total</small><strong>{result.total!==undefined?`R$ ${money(result.total)}`:"—"}</strong></div>{(result.requiredProducts||result.ct1RequiredProducts)?.length>0&&<p>Produtos adicionados: {(result.requiredProducts||result.ct1RequiredProducts).map((item:any)=>`${item.code} (${item.quantity}x)`).join(" • ")}</p>}{result.warranty?.message&&<p>{result.warranty.message}{result.warranty.cartTotalAfterWarranty!==undefined?` Total usado para escolher a faixa: R$ ${money(result.warranty.cartTotalAfterWarranty)}.`:""}</p>}{result.message&&<p>{result.message}</p>}</div>}
