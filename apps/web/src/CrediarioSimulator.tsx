...
const phaseNames=["Iniciando sessão da Plataforma Click","Localizando o produto","Adicionando o produto principal","Adicionando o primeiro produto auxiliar","Adicionando o produto auxiliar 801911","Verificando / aplicando garantia","Selecionando o seguro prestamista","Enviando CPF","Configurando a condição de pagamento","Aplicando entrada variável","Finalizando consulta"] as const;
...
export function CrediarioSimulator(){
 ...
 useEffect(()=>{if(!loading){setProgressIndex(-1);return}setProgressIndex(0);const timer=window.setInterval(()=>setProgressIndex(current=>Math.min(current+1,phaseNames.length-2)),1200);return()=>window.clearInterval(timer);},[loading]);
 ...
 return <section className="creditSimulator">
  ...
  {loading&&<div className="creditProgress" aria-live="polite"><div className="creditProgressTop"><strong>{currentPhase}</strong><span>{progressPercent}%</span></div><div className="creditProgressBar"><span style={{width:`${progressPercent}%`}}/></div><div className="creditProgressList">{phaseNames.map((phase,index)=><div key={phase} className={index<progressIndex?"done":index===progressIndex?"active":"pending"}><span>{index<progressIndex?"✓":index===progressIndex?"●":"○"}</span>{phase}{phase.includes("801911")&&<b>{auxiliaryQuantity}x</b>}</div>)}</div></div>}
