...
function publicError(e:unknown){
 ...
 const known=new Set([
  "CLICK_CREDENTIALS_NOT_CONFIGURED","CLICK_INTERACTIVE_CHALLENGE_REQUIRED","CLICK_SESSION_EXPIRED","CLICK_LOGIN_FAILED",
...
app.post("/api/session/refresh",auth,async(_req,res)=>{
 try{await refreshClickSession();res.json({ok:true});}
...
app.get("/api/products/:code",auth,async(req,res)=>{
 ...
 try{res.json(await withAuthenticatedClickPage(page=>searchProduct(page,code)));}
 catch(e){console.error(`[GET /api/products/${code}]`,e);res.status(500).json({error:publicError(e)});}
});
