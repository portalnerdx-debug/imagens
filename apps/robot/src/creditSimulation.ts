...
async function chooseVoltageIfRequested(page:Page,voltage?:string){
 ...
 if(modalVisible){
  ...
  if(!await option.isVisible({timeout:2000}).catch(()=>false))throw new Error("VOLTAGE_OPTION_NOT_FOUND");
  await option.click();
...
async function chooseWarranty(page:Page,warranty:boolean){
 ...
 if(await form.count()){
  const selector=warranty?'input[name="op_garantia"][id*="12" i]':'input[name="op_garantia"][value="0"]';
  ...
  if(await option.count()){
   ...
   if(productCode){
    ...
    return {shown:true,selected:warranty,serviceCode,months:warranty?12:0};
   }
  }
 }
 if(warranty){
  const twelveMonthLink=page.locator('a[href*="op_garantia="]').filter({hasText:/12\s*(?:mes(?:es)?|m\b)/i}).first();
 ...
 if(chosenHref){
  for(let index=0;index<count;index++){
   ...
   if(await chosen.isVisible({timeout:1000}).catch(()=>false)){
    ...
    return {shown:true,selected:warranty,serviceCode:warrantyOptionCode(chosenHref)||undefined,months:warranty?12:0};
   }
  }
  await page.goto(new URL(chosenHref.replace(/&amp;/g,"&"),page.url()).toString(),{waitUntil:"domcontentloaded",timeout:60000});
  await page.waitForTimeout(300);
  return {shown:true,selected:warranty,serviceCode:warrantyOptionCode(chosenHref)||undefined,months:warranty?12:0};
 }
...
async function addRequiredCartItems(page:Page,items:Ct1RequiredItem[]){
 ...
 for(const item of items){
  ...
  if(!alreadyInCart){
   const added=await page.request.get(cartUrl,{params:{cod:item.code,acao:"incluir"},headers,timeout:60000});
   if(!added.ok())throw new Error("TRADITIONAL_CART_PREPARATION_FAILED");
   cartHtml=await added.text();
  }
  if(item.unitPrice!==undefined){
   const quantity=await page.request.get(`${origin}/checkout_catalogo/carrinho_quantidade_ajax.php`,{params:{cod:item.code,qtd:String(item.quantity),vl_venda:String(item.unitPrice),prevent_cache:new Date().toString(),_:String(Date.now())},headers,timeout:60000});
...
async function prepareBaseCreditCart(page:Page,plan:CreditPlan,auxiliaryQuantity=3){
 return addRequiredCartItems(page,baseRequiredItemsForPlan(plan,auxiliaryQuantity));
}
...
async function preparePrestamistaBand(page:Page,totalWithoutPrestamista:number){
 ...
 for(const code of PRESTAMISTA_CODES){
  const removed=await page.request.get(cartUrl,{params:{cod:code,acao:"excluir"},headers,timeout:60000});
  if(!removed.ok())throw new Error("PRESTAMISTA_BAND_SETUP_FAILED");
 }
 const added=await page.request.get(cartUrl,{params:{cod:selected.code,acao:"incluir"},headers,timeout:60000});
 ...
 if(present.length!==1||present[0]!==selected.code)throw new Error("PRESTAMISTA_BAND_SETUP_FAILED");
...
async function applyTraditionalWarrantyService(page:Page,productCode:string,enabled:boolean,selectedCode?:string){
 ...
 const response=await page.request.get(`${origin}/checkout_catalogo/processa_servico_ajax.php`,{params:{cd_itprodd:productCode,cd_servico:serviceCode,op_servico:"Y",_:String(Date.now())},headers:{referer:page.url(),"x-requested-with":"XMLHttpRequest"},timeout:60000});
 ...
 return {shown:true,selected:enabled,serviceCode,cartTotalAfterWarranty:cartTotal,message:enabled?"Garantia adicional aplicada ao produto principal.":"Produto principal mantido sem garantia adicional."};
}

async function simulateCt1(page:Page,req:CreditRequest,context:{product:unknown;voltage:unknown;warranty:unknown;requiredProducts?:Array<{code:string;quantity:number;reason:string}>}){
...
async function browserAjaxGet(page:Page,endpoint:string,params:Record<string,string>){
 return page.evaluate(async({endpoint,params})=>{
  ...
  const response=await fetch(url.toString(),{method:"GET",credentials:"include",cache:"no-store",headers:{"Accept":"application/json, text/javascript, */*; q=0.01","X-Requested-With":"XMLHttpRequest"}});
...
function variableEntryError(plan:CreditPlan,suffix:string){return `${plan}_${suffix}`;}

async function simulateVariableEntry(page:Page,req:CreditRequest,context:{product:unknown;voltage:unknown;warranty:unknown;requiredProducts?:Array<{code:string;quantity:number;reason:string}>}){
 ...
 const variable=await browserAjaxGet(page,variableEndpoint,{cod_pagto:req.plan,_:String(Date.now())});
 ...
 let paymentId=paymentIdFromSetup||paymentIdFromSetupPage||paymentIdFromVariable||await readPaymentEntryId(page,origin,req.plan);
 let rebuiltPayload="";
 if(!paymentId){
 ...
 const entry=await browserAjaxGet(page,`${origin}/checkout_catalogo/processa_inclui_pagamento_variavel_ajax.php`,{cd_pagamento:paymentId,valor:formatBrazilianMoney(downPayment),_:String(Date.now())});
...
export async function simulateCredit(page:Page,req:CreditRequest){
 ...
 if((req.plan==="CT2"||req.plan==="48")&&(req.installments<2||req.installments>24))throw new Error(variableEntryError(req.plan,"INSTALLMENTS_OUT_OF_RANGE"));
 ...
 const auxiliaryQuantity=Number.isInteger(req.auxiliaryQuantity)&&Number(req.auxiliaryQuantity)>=1&&Number(req.auxiliaryQuantity)<=20?Number(req.auxiliaryQuantity):3;
 ...
 if(req.plan==="CT1")return simulateCt1(page,req,{product,voltage,warranty,requiredProducts});
 if(req.plan==="CT2"||req.plan==="48")return simulateVariableEntry(page,req,{product,voltage,warranty,requiredProducts});
